...
function User_Destroy(username,revision,cb)
{
	blah.destroy(username, revision, function(err, body) {
  if (!err)
    {	console.log(body);
	//cb(true);
}
...
{
	blah.insert({"name": groupname,"owner": owner,"type": "group","members": []}, groupname, function(err, body) {
  if (!err)
    {	console.log(body);
	//cb(true);
}
...
function Group_Destroy(groupname,revision,cb)
{
	blah.destroy(groupname, revision, function(err, body) {
  if (!err)
    {	console.log(body);
	//cb(true);
}
